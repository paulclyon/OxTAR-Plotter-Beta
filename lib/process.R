      ...
      #if (!is.na(ptForRx) & ptForRx == "1" & !is.na(ptOffPathway) & ptOffPathway == "0")
      {
        ...
        if (!is.na(ref_date)) {
          ...
          if (!is.na(ref_rx_date))
          {
            ...
            # If the Ref is NA then set it to the Rx date as next best guess... Otherwise it may be missed from OxTAR & Audit
            if (is.na(ref_date))
            {
              ref_date <- ref_rx_date
            }
            
            # If the DTT is NA then set it to the Rx date as next best guess...Otherwise it may be missed from OxTAR & Audit
            if (is.na(ref_dtt_date))
            {
